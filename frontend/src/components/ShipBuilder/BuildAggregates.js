...
const BuildAggregates = () => {
  ...
  useEffect(() => {
    !_.isEmpty(state.currentBuild) && (
      calcAggregates(),
      dispatch({ type: 'setDefenseAggregates', payload: { ...defense.current } }),
      dispatch({ type: 'setMobilityAggregates', payload: { ...mobility.current } }),
      dispatch({ type: 'setMissionsAggregates', payload: { ...missions.current } }),
      ...
      dispatch({ type: 'setOffenseAggregates', payload: { ...offense.current } })
    )   
  }, [state.currentBuild])
  ...
  const calcAggregates = () => {
    ...
    // Anti-missile DPS
    defense.current.anti_missile_dps = getBuildAttribute(build, 'anti_missile_dps')
    defense.current.anti_missile_dps_contributers = build.outfits.map((outfit_set) => {
      if (outfit_set.outfit.anti_missile > 0){
        let tip = ''
        for (let i = 0; i < outfit_set.amount; i++) {
          tip += `<p>${outfit_set.outfit.name}: ${outfit_set.outfit.shots_per_second} shots/s \u00d7 ${outfit_set.outfit.anti_missile} anti-missile = ${outfit_set.outfit.shots_per_second * outfit_set.outfit.anti_missile}</p>`
        }
        return tip
      }
    }).join('')
    ...
    // Empty mass
    const emptyMass = mobility.current.mass_no_cargo = getBuildAttribute(build, 'mass')

    // Thrust
    const thrust = mobility.current.thrust = getBuildAttribute(build, 'thrust')
    ...
    // Cargo space
    const cargoSpace = missions.current.cargo_space = getBuildAttribute(build, 'cargo_space')
    ...
    // Fuel capacity
    const fuelCapacity = mobility.current.fuel_capacity = getBuildAttribute(build, 'fuel_capacity')

    // Jump fuel
    const jumpFuel = mobility.current.jump_fuel = build.outfits.reduce((previous, current) => Math.max(previous, current.outfit.jump_fuel), 0)

    // Firing fuel
    const firingFuel = mobility.current.firing_fuel = getBuildAttribute(build, 'fuel_per_second')
    
    // Ramscoop
    const ramscoop = mobility.current.ramscoop = getBuildAttribute(build, 'ramscoop')
    ...
    space.current.total_outfit_space = build.outfits.reduce(((a,b) => a + (b.amount * ((parseFloat(b.outfit.outfit_space) > 0 && parseFloat(b.outfit.outfit_space)) || 0))), parseFloat(build.hull.outfit_space) || 0)
    ...
    space.current.free_engine_capacity = getBuildAttribute(build, 'engine_capacity')

    // Total weapon capacity
    space.current.total_weapon_capacity = parseFloat(build.hull.weapon_capacity)
    // Remaining weapon capacity
    space.current.free_weapon_capacity = getBuildAttribute(build, 'weapon_capacity')
    ...
    const coolingEfficiency = heat.current.cooling_efficiency = 2 + 2 / (1 + Math.exp(coolingInefficiency/-2)) -4 / (1 + Math.exp(coolingInefficiency/-4))

    // Cooling
    const cooling = heat.current.cooling = getBuildAttribute(build, 'cooling') * coolingEfficiency 
    // Active Cooling
    const activeCooling = heat.current.active_cooling = getBuildAttribute(build, 'active_cooling') * coolingEfficiency

    // Idle heat generation
    const heatGeneration = heat.current.heat_generation = getBuildAttribute(build, 'heat_generation')
    
    // Idle heat balance
    const idleHeat = heat.current.idle_heat = (heatGeneration - cooling - activeCooling)

    // Thrusting heat
    const thrustingHeat = heat.current.thrusting_heat = getBuildAttribute(build, 'thrusting_heat') 
    // Reverse thrusting heat
    const reverseThrustingHeat = heat.current.reverse_thrusting_heat = getBuildAttribute(build, 'reverse_thrusting_heat') 
    // Turning heat
    const turningHeat = heat.current.turning_heat = getBuildAttribute(build, 'turning_heat') 
    // Afterburner heat
    const afterburnerHeat = heat.current.afterburner_heat = getBuildAttribute(build, 'afterburner_heat') 
        
    // Moving heat
    const movingHeat = heat.current.moving_heat =
      (Math.max(thrustingHeat, reverseThrustingHeat) + turningHeat + afterburnerHeat)
    
    // Firing heat
    const firingHeat = heat.current.firing_heat = getBuildAttribute(build, 'heat_per_second')

    // Shield regen heat
    const shieldHeat = heat.current.shield_heat = getBuildAttribute(build, 'shield_heat') 
    // Hull regen heat
    const hullHeat = heat.current.hull_heat = getBuildAttribute(build, 'hull_heat') 

    // Heat balance in action
    const actionHeat = heat.current.action_heat = idleHeat + movingHeat + firingHeat + shieldHeat + hullHeat

    // Heat dissipation
    const heatDissipation = heat.current.heat_dissipation = getBuildAttribute(build, 'heat_dissipation') * 6

    // Maximum heat
    const maximumHeat = heat.current.max_heat = emptyMass * heatDissipation

    // Idle heat level
    const idleHeatLevel = heat.current.idle_heat_level = idleHeat > 0 ? idleHeat / maximumHeat : 0

    // Action heat level
    const actionHeatLevel = heat.current.action_heat_level = actionHeat > 0 ? actionHeat / maximumHeat : 0

    
    // Energy info
    // Energy generation
    const energyGeneration = energy.current.energy_generation = getBuildAttribute(build, 'energy_generation') 

    // Solar collection
    const solarCollection = energy.current.solar_collection = getBuildAttribute(build, 'solar_collection') 

    // Energy consumption
    const energyConsumption = energy.current.energy_consumption = getBuildAttribute(build, 'energy_consumption') 

    // Cooling energy
    const coolingEnergy = energy.current.cooling_energy = getBuildAttribute(build, 'cooling_energy') 

    // Idle energy
    const idleEnergy = energy.current.idle_energy = energyGeneration + solarCollection - energyConsumption - coolingEnergy

    // Thrusting energy
    const thrustingEnergy = energy.current.thrusting_energy = getBuildAttribute(build, 'thrusting_energy') 
    // Reverse-thrusting energy
    const reverseThrustingEnergy = energy.current.reverse_thrusting_energy = getBuildAttribute(build, 'reverse_thrusting_energy') 
    // Turning energy
    const turningEnergy = energy.current.turning_energy = getBuildAttribute(build, 'turning_energy') 
    // Afterburner energy
    const afterburnerEnergy = energy.current.afterburner_energy = getBuildAttribute(build, 'afterburner_energy') 

    // Moving energy
    const movingEnergy = energy.current.moving_energy = Math.max(thrustingEnergy, reverseThrustingEnergy) + turningEnergy + afterburnerEnergy

    // Firing energy
    const firingEnergy = energy.current.firing_energy = getBuildAttribute(build, 'energy_per_second')

    // Shield regen energy
    const shieldEnergy = energy.current.shield_energy = getBuildAttribute(build, 'shield_energy') 

    // Hull regen energy
    const hullEnergy = energy.current.hull_energy = getBuildAttribute(build, 'hull_energy') 

    // Energy in action
    const actionEnergy = energy.current.action_energy = idleEnergy - movingEnergy - firingEnergy - shieldEnergy - hullEnergy

    // Energy capacity
    const energyCapacity = energy.current.energy_capacity = getBuildAttribute(build, 'energy_capacity')


    // Damage
    // Shield DPS
    const shieldDps = offense.current.shield_dps = getBuildAttribute(build, 'shield_dps')

    // Hull DPS
    const hullDps = offense.current.hull_dps = getBuildAttribute(build, 'hull_dps')

    // Average DPS
    const averageDps = offense.current.average_dps = (shieldDps + hullDps) / 2

    // Heat DPS
    const heatDps = offense.current.heat_dps = getBuildAttribute(build, 'heat_dps')

    // Ion DPS
    const ionDps = offense.current.ion_dps = getBuildAttribute(build, 'ion_dps')

    // Slowing DPS
    const slowingDps = offense.current.slowing_dps = getBuildAttribute(build, 'slowing_dps')

    // Disruption DPS
    const disruptionDps = offense.current.disruption_dps = getBuildAttribute (build, 'disruption_dps')
    ...
    rows: [
      {
        ...
        values: [state.energyAggregates.energy_generation + state.energyAggregates.solar_collection],
      },
      {
