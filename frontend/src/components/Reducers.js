...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'filterRelease':
      return { ...state, release: action.payload, outfitSort: 'name', hullSort: 'name' }
    case 'filterSpoiler':
      return { ...state,
    ...
    case 'sortHulls': {
      ...
      // Else, sort by new attribute
        ...
        if (['cost', 'drag', 'name', 'mass', 'required_crew'].includes(sortAttribute)) {
          relevantSelection = relevantSelection.sort(fieldSorter([sortAttribute]))
        }
        else {
          relevantSelection = relevantSelection.sort(fieldSorter(['-' + sortAttribute]))
        }
      ...
      return state.hullSearchQuery ? 
        { ...state, hullSearchResults: relevantSelection, hullSort: sortAttribute } : 
        { ...state, currentHulls: relevantSelection, hullSort: sortAttribute }
    }
    case 'searchHulls':
    ...
    case 'sortOutfits': {
      ...
      // Else, sort by new attribute
        ...
        if (['cost', 'energy_per_second', 'firing_energy', 'firing_heat', 'heat_generation', 'heat_per_second', 'inaccuracy', 'name', 'mass', 'reload_time', 'thrusting_energy', 'thrusting_heat', 'turning_energy', 'turning_heat'].includes(sortAttribute)) {
          relevantSelection = relevantSelection.sort(fieldSorter([sortAttribute]))
        }
        else {
          relevantSelection = relevantSelection.sort(fieldSorter(['-' + sortAttribute]))
      ...
      return state.outfitSearchQuery ? 
        { ...state, outfitSearchResults: relevantSelection, outfitSort: sortAttribute } : 
        { ...state, currentOutfits: relevantSelection, outfitSort: sortAttribute }
    }
    case 'searchOutfits':
    ...
    case 'setDefaultBuild': {
      ...
      return { ...state, currentBuild: currentBuild, defaultBuild: parsedBuild }
    }
    case 'resetDefaultBuild':
      return { ...state, defaultBuild: {} }
    case 'setHullBuilds': {
      const hullBuilds = state.allBuilds.filter(build => build.hull === state.currentHull.id)
      const parsedHullBuilds = hullBuilds.map(build => parseBuild(cloneDeep(build)))
    ...
    case 'deleteBuild': {
      ...
      newSavedHullBuilds.splice(state.savedHullBuilds.indexOf(action.payload), 1)
      return { ...state, savedHullBuilds: newSavedHullBuilds}
    }
    case 'clearBuild':
